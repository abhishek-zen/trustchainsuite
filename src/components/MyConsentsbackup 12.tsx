...
import { 
  Search, Filter, Calendar, ChevronDown, Shield, Edit, 
      ...
      {visibleItems.map((item, i) => (
        <span
          ...
          className={`inline-block ${
            type === 'field' ? 'bg-slate-100 text-slate-700' : 'bg-indigo-100 text-indigo-700'
...
const ConsentMatrixDialog = ({ open, onClose, onSave, editRow, setEditRow }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'} transition-opacity bg-slate-900/50`}>
      <div className="bg-white rounded-xl shadow-xl w-11/12 max-w-6xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-auto p-6 flex-grow">
          <h4 className="text-slate-800 font-medium mb-4">Field-Level Consent Matrix</h4>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 text-slate-600">
                    <th className="py-3 px-4 text-left font-medium border-b border-slate-200">Field</th>
                    {purposes.map((purpose, colIdx) => (
                      <th key={colIdx} className="py-3 px-4 text-center font-medium border-b border-slate-200">
                        {purpose}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {fields.map((field, rowIdx) => (
                    <tr key={rowIdx} className="hover:bg-slate-50 transition-all">
                      <td className="py-3 px-4 text-slate-800 border-b border-slate-100">{field}</td>
                      {purposes.map((_, colIdx) => (
                        <td key={`fp-${rowIdx}-${colIdx}`} className="py-3 px-4 text-center border-b border-slate-100">
                          <input
                            type="checkbox"
                            className="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 rounded focus:ring-indigo-500 focus:ring-2"
                            checked={editRow[`fp_${rowIdx}_${colIdx}`] || false}
                            onChange={() => handleFieldPurposeToggle(rowIdx, colIdx)}
                          />
                        </td>
                      ))}
                    </tr>
                ...
                <tfoot>
                  <tr className="bg-slate-50 font-medium">
                    ...
                    {purposes.map((_, colIdx) => (
                      <td key={`selectAll-${colIdx}`} className="py-3 px-4 text-center">
                        <input
                          type="checkbox"
                          className="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 rounded focus:ring-indigo-500 focus:ring-2"
                          checked={fields.every((_, rowIdx) => editRow[`fp_${rowIdx}_${colIdx}`])}
                          onChange={(e) => handleColumnSelectAll(colIdx, e.target.checked)}
                        />
                      </td>
                    ))}
                  </tr>
      ...
      .then(ab => {
        ...
        const parsed = json.map(row => {
          ...
          for (const key in headerMap) {
            ...
            entry[key] = typeof row[col] === 'string' ? row[col].trim() : row[col];
          }
          entry.fields = entry.fields?.replace(/[{}]/g, '');
          entry.status = entry.status || 'Pending';
          
          // Format the date
          entry.expiry = typeof entry.expiry === 'number'
            ? format(new Date(Math.round((entry.expiry - 25569) * 86400 * 1000)), 'MMM dd, yyyy')
            : entry.expiry;
        ...
        // Calculate counts
        setActiveCount(parsed.filter(item => item.status === 'Approved').length);
        setRevokedCount(parsed.filter(item => item.status === 'Revoked').length);
        setPendingCount(parsed.filter(item => item.status === 'Pending').length);
        
        setLoading(false);
      })
      .catch(() => {
        setError('Unable to load data.xlsx. Ensure it is placed in the public/ directory.');
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans">
      ...
      <div className="px-6 py-8">
        {/* Page header */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-slate-800 mb-2">Consent Dashboard</h2>
          <p className="text-slate-500">Monitor and manage all data access consents across applications</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white rounded-xl shadow-md p-6 transition-all hover:shadow-lg">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-slate-500 text-sm font-medium">Active Consents</p>
                <p className="text-3xl font-semibold text-slate-800 mt-2">{activeCount}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 transition-all hover:shadow-lg">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-slate-500 text-sm font-medium">Revoked Consents</p>
                <p className="text-3xl font-semibold text-slate-800 mt-2">{revokedCount}</p>
              </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
            ...
            <div className="relative">
              ...
              <div className="absolute mt-1 w-48 bg-white rounded-lg shadow-lg border border-slate-200 hidden">
                {['All', 'Approved', 'Pending', 'Rejected', 'Revoked'].map(status => (
                  <button 
                    key={status}
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50">
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Application</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Dataset</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Classification</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Fields</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Purpose</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Expiry</th>                  
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">CISO</th>
                    <th className="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">CPO</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-all">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-800">{row.app}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{row.dataset}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{row.classification}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">
                        <ExpandableText text={row.fields} limit={3} type="field" />                        
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-600">
                        <ExpandableText text={row.purpose} limit={2} type="purpose" />
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                        <div className="flex flex-col">
                          <span>{row.expiry}</span>
                          {row.daysRemaining > 0 && (
                            <span className={`text-xs ${row.daysRemaining < 30 ? 'text-amber-600' : 'text-emerald-600'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          row.ciso === 'Approved' ? 'bg-emerald-50 text-emerald-700' : 
                          row.ciso === 'Pending For Approval' ? 'bg-amber-50 text-amber-700' : 
                          row.ciso === 'Revoked' ? 'bg-rose-50 text-rose-700' :
                          row.ciso === 'Rejected' ? 'bg-rose-50 text-rose-700' :
                          'bg-slate-50 text-slate-700'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                            row.ciso === 'Approved' ? 'bg-emerald-500' : 
                            row.ciso === 'Pending For Approval' ? 'bg-amber-500' : 
                            row.ciso === 'Revoked' ? 'bg-rose-500' :
                            row.ciso === 'Rejected' ? 'bg-rose-500' :
                            'bg-slate-500'
                          }`}></span>
                          {row.ciso}
                        </span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          row.cpo === 'Approved' ? 'bg-emerald-50 text-emerald-700' : 
                          row.cpo === 'Pending For Approval' ? 'bg-amber-50 text-amber-700' : 
                          row.cpo === 'Revoked' ? 'bg-rose-50 text-rose-700' :
                          row.cpo === 'Rejected' ? 'bg-rose-50 text-rose-700' :
                          'bg-slate-50 text-slate-700'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                            row.cpo === 'Approved' ? 'bg-emerald-500' : 
                            row.cpo === 'Pending For Approval' ? 'bg-amber-500' : 
                            row.cpo === 'Revoked' ? 'bg-rose-500' :
                            row.cpo === 'Rejected' ? 'bg-rose-50 text-rose-700' :
                            'bg-slate-500'
                          }`}></span>
                          {row.cpo}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          row.status === 'Pending For Approval' ? 'bg-amber-50 text-amber-700' : 
                          row.status === 'Revoked' ? 'bg-rose-50 text-rose-700' :
                          'bg-slate-50 text-slate-700'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                            row.status === 'Approved' ? 'bg-emerald-500' : 
                            row.status === 'Pending For Approval' ? 'bg-amber-500' : 
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          {row.status === 'Pending' ? (
                            <button
                              ...
                              className="px-3 py-1 bg-emerald-50 text-emerald-700 text-xs rounded-md hover:bg-emerald-100 transition-colors"
                            >
                              Approve
                            </button>
                          ) : row.status === 'Approved' ? (
                            <button
                              ...
                              className="px-3 py-1 bg-rose-50 text-rose-700 text-xs rounded-md hover:bg-rose-100 transition-colors"
                            >
                              Revoke
                            </button>
                          ) : null}
